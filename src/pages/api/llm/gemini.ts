...
import { GOOGLE_AI_MODEL_ID, GOOGLE_API_KEY, RESUME_CACHE_DURATION, RESUME_URL } from "astro:env/server";
...
const model = genAI.getGenerativeModel({ model: GOOGLE_AI_MODEL_ID, systemInstruction: systemInstructions });
...
export const POST: APIRoute = async ({ request }) => {
    try {
        ...
        const result: string = (await model.generateContent([message, resumeData])).response.text();
